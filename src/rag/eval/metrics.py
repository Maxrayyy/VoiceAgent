...
def evaluate_retrieval(
    ...
    for item in queries:
        ...
        for i, content in enumerate(retrieved_contents):
            if golden in content:
                relevant_indices.add(i)

        retrieved_ids = list(range(len(retrieved_contents)))
        ...
        per_query.append({
            ...
            "hit": hr > 0,
            "rank": next((i + 1 for i in range(len(retrieved_contents)) if golden in retrieved_contents[i]), -1),
