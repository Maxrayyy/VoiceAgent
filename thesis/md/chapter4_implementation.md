...
在这一划分方式下，浏览器接入层承担前端连接建立、消息收发和会话承载职责，并与会话编排逻辑共同构成实现层面的控制中枢。这种划分方式与第三章的设计完全一致，因此后续实现叙述能够做到逐项映射，保证系统设计与系统实现保持统一。
...
从表面上看，这一流程仍然遵循“识别、检索、生成、合成、播放”的处理顺序，但其真正价值在于尽可能让相邻阶段在时间上形成重叠。也就是说，系统实现关注的重点并不是孤立地压缩某一个模块的耗时，而是通过流式衔接降低整条链路的用户感知延迟。正因为如此，后文各模块的实现都会围绕“减少等待”和“保持可中断性”展开。
...
这种索引组织方式的价值并不体现在文件形式本身，而体现在不同能力之间的相互配合。向量索引负责提供语义召回能力，BM25 索引负责强化专业术语的精确匹配，而文档元数据则承担来源恢复与结果解释功能。只有在三者共同作用的情况下，RAG 模块才能既完成有效召回，又向用户提供可核验的依据来源。
...
将 Token 管理从识别流程中单独抽离后，识别会话本身就不再需要反复处理鉴权刷新细节，从而能够保持更清晰的职责边界；与此同时，如果后续需要调整鉴权方式或刷新策略，也不会直接影响识别回调与音频传输逻辑。对于需要持续运行的语音问答服务而言，这种分层处理能够使整体结构更加稳定，也更便于后续维护。
...
因此，本项目将 WebSocket 作为会话编排与服务接入模块中的通信承载机制，由服务接入层负责接收外部输入，再交由会话编排逻辑统一处理。通过这种实现方式，浏览器通信细节能够被收敛在接入层内部，识别、检索、生成和合成模块也得以保持清晰的职责边界。
